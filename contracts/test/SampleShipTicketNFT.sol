...
contract SampleShipTicketNFT is NFTokenMetadata, Ownable {
    ...
    function mint(
        ...
        require(MintedNFT <= maxMintAllowed, 'Maximum number of tickets minted');
        MintedNFT += 1;
        _mint(_to, _tokenId);
        _setTokenUri(_tokenId, _uri);
        OwnedTickets[_to].push(_tokenId);
    }

    function transfer(address to) external {
        uint256 len = OwnedTickets[msg.sender].length;
        require(len > 0, 'you do not have any tickets');
        this.safeTransferFrom(msg.sender, to, OwnedTickets[msg.sender][len - 1]);
        OwnedTickets[to].push(OwnedTickets[msg.sender][len - 1]);
        OwnedTickets[msg.sender].pop();
    }

    function checkOwnedTickets(address account) external view returns (uint256[] memory) {
        return OwnedTickets[account];
    }
}
